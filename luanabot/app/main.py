...
@app.route('/api/banco/debug_tx', methods=['GET'])
def api_banco_debug_tx():
    """Diagnóstico: mostra exatamente o que o Enable Banking devolve para as
    contas Revolut do utilizador, sem esperar pelo ciclo de 30 min do scheduler.
    Usar: /api/banco/debug_tx?t=zef2026&phone=264909371768998&minutos=120"""
    token_acesso = request.args.get('t','')
    if token_acesso != 'zef2026':
        return jsonify({'error': 'acesso negado'}), 401
    ...
    try:
        ...
        contas_dbg = db.session.execute(text(
            "SELECT account_id, banco FROM bancos_ligados "
            "WHERE usuario_id=:u AND ativo=TRUE AND account_id IS NOT NULL AND banco NOT LIKE '%cofre%'"),
            {'u': usuario_dbg.id}).fetchall()
        ...
        for acc_id, banco in contas_dbg:
            info_conta = {'banco': banco, 'account_id': acc_id}
            try:
                ...
                info_conta['excecao'] = str(e)
            resultado['por_conta'].append(info_conta)
    except Exception as e:
...
def processar_texto(phone_raw, phone, texto):
    try:
        ...
        if estado == 'confirmar_aviso_divida':
            d_ad = dados_estado
            ...
            if tem_sim_ad:
                ...
                if parceiro_phone_ad and pessoa_ad.lower() == nome_parceiro_ad:
                    meu_nome_ad = NOMES_CASAL.get(usuario.phone, 'Alguem')
                    notificar_parceiro(usuario.phone, f"👀 Lembrete: ainda deves {total_ad:.0f}€ a {meu_nome_ad} 😉")
        ...
        if estado == 'aviso_aniversario_leve':
            ...
            if tem_nao_av or tem_sim_av:
                ...
                enviar_mensagem(phone_raw,
                    ...
                    f"diz 'prenda {nome_av} 20€' (ou o valor que quiseres) 🎁")
            else:
                processar_texto(phone_raw, phone, texto)
            return
        ...
        # ── JÁ PAGUEI [A/AO/DA/DO + PESSOA] (eu liquidei o que devia) ──
        # Robusto a várias frases: "já paguei à Luana", "já paguei os 500 da Luana",
        # "já paguei o Vitor" — basta "já paguei" + o nome aparecer na frase.
        pessoa_paga = None
        if re.search(r'\b(?:ja|já)\s+paguei\b', t):
            parceiro_phone_jp0 = get_parceiro_phone(usuario.phone)
            nome_parceiro_jp0 = NOMES_CASAL.get(parceiro_phone_jp0, '') if parceiro_phone_jp0 else ''
            if nome_parceiro_jp0 and nome_parceiro_jp0.lower() in t:
                pessoa_paga = nome_parceiro_jp0
            else:
                m_ja_paguei = re.search(r'(?:ao|à|a|da|do|de|o|os)\s+([A-Za-zÀ-ú]{2,})\s*$', texto.strip(), re.IGNORECASE)
                if not m_ja_paguei:
                    m_ja_paguei = re.search(r'(?:ao|à|a|da|do|de|o|os)\s+([A-Za-zÀ-ú]{2,})\b', texto, re.IGNORECASE)
                if m_ja_paguei:
        ...
        if pessoa_paga:
            try:
                ...
                if r:
                    ...
                    enviar_mensagem(phone_raw, f"OK! Marquei como pago — {r[1]:.2f}€ ao {pessoa_paga} ({desc_limpa})")
                    ...
                    saldo_dp, parcela_dp = get_saldo_divida(usuario.id, pessoa_paga)
                    if saldo_dp > 0:
                        valor_pago_dp = extrair_valor(texto) or parcela_dp
                        novo_saldo_dp = max(0, saldo_dp - valor_pago_dp)
                        ...
                        try:
                            ...
                            db.session.rollback()
                        meu_nome_dp = NOMES_CASAL.get(usuario.phone, 'Parceiro')
                        if novo_saldo_dp > 0:
                            meses_rest_dp = int(novo_saldo_dp/parcela_dp) + (1 if novo_saldo_dp % parcela_dp > 0 else 0)
                            enviar_mensagem(phone_raw,
                                f"✅ Pago {valor_pago_dp:.0f}€ à {pessoa_paga}\n💳 Saldo restante: {novo_saldo_dp:.0f}€\n📅 ~{meses_rest_dp} mes(es) para terminar")
                            notificar_parceiro(usuario.phone,
                                f"💸 {meu_nome_dp} pagou {valor_pago_dp:.0f}€ da dívida\n💳 Falta: {novo_saldo_dp:.0f}€\n📅 ~{meses_rest_dp} mes(es) para terminar")
                        else:
                            enviar_mensagem(phone_raw, f"🎉 Dívida à {pessoa_paga} paga! Zero euros em dívida 💪")
                            notificar_parceiro(usuario.phone, f"🎉 {meu_nome_dp} pagou a dívida toda! Estão quites 💪")
                    else:
                        enviar_mensagem(phone_raw, f"Não encontrei nenhuma dívida tua ao {pessoa_paga} em aberto 🤔\nSe queres só registar o gasto, diz o valor de outra forma.")
            except Exception as e:
        ...
        for u in Usuario.query.all():
            ...
            try:
                ...
                for pessoa, total, n in rows:
                    perguntar_ou_enfileirar(u.phone, 'confirmar_aviso_divida', {'pessoa': pessoa, 'total': total},
                        f"👀 {pessoa} ainda te deve {total:.0f}€ "
                        f"({n} conta{'s' if n>1 else ''} há mais de 7 dias)\n"
                        f"Queres mandar-lhe um aviso? (sim/não)")
            except Exception as e:
    ...
    with app.app_context():
        ...
        try:
            ...
            for r in rows:
                ...
                if dias == 5:
                    perguntar_ou_enfileirar(phone, 'aviso_aniversario_leve', {'nome': nome},
                        f"🎂 Daqui a 5 dias e o aniversario de {nome}! Ja pensaste no presente? 🎁")
                elif dias == 1:
                    perguntar_ou_enfileirar(phone, 'aviso_aniversario_leve', {'nome': nome},
                        f"🎂 AMANHA e o aniversario de {nome}! Nao te esqueças! 🎉")
                elif dias == 0:
                    perguntar_ou_enfileirar(phone, 'aviso_aniversario_leve', {'nome': nome},
                        f"🎂🎉 HOJE e o aniversario de {nome}! Ja desejaste? 💕")
